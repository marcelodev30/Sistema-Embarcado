...
esp_err_t ds3231_init_desc(i2c_dev_t *dev, i2c_port_t port, int sda_gpio, int scl_gpio) {
    CHECK_ARG(dev);
...
esp_err_t ds3231_set_time(i2c_dev_t *dev, struct tm *time) {
    ...
    // The week data must be 1 to 7 to keep the start of week the same day as for tm_wday (1 is sunday)
...
esp_err_t ds3231_get_time(i2c_dev_t *dev, struct tm *time) {
    ...
    else time->tm_hour = bcd2dec(data[2]);
